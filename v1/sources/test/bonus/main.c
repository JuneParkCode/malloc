...
int main()
{
	...
	for (int i = 0; i < thread_size; ++i) {
		arg[i].count = 16;
		if (pthread_create(&thread_t[i], NULL, test_tiny_size, (void *)&arg) <
			0) {
	...
	for (int i = 0; i < thread_size; ++i) {
		arg[i].count = 16;
		if (pthread_create(&thread_t[i], NULL, test_small_size, (void *)&arg) <
			0) {
			perror("thread create error:");
			return (-1);
		}
	...
	for (int i = 0; i < thread_size; ++i) {
		arg[i].count = 16;
		if (pthread_create(&thread_t[i], NULL, test_large_size, (void *)&arg) <
			0) {
			perror("thread create error:");
			return (-1);
		}
